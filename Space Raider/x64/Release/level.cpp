...
void Level::generate(const sf::RenderWindow* window, int lvl_nr) {
    ...
    if (lvl_nr == 6) {
        ...
        for (int j = 0; j < 3; j++) {
            for (int i = 0; i < 5; i++) {
                enemies_.emplace_back(new  Enemy("Textures/enemy1.png", 1, sf::IntRect(0, 0, 216, 63), 100 * i + 25, window->getSize().y - (400 + 100 * j), false, 6, -105));
                actors_.emplace_back(enemies_[enemies_.size() - 1]);
            }
        }
    }
    if (lvl_nr == 7) {
        clear_actors();
        scoreboads_[0]->update_score(100);
        enemies_.emplace_back(new  Enemy("Textures/enemy.png", 0.4, sf::IntRect(0, 0, 590, 1180), window->getSize().x / 2, 100, false, 100, -75, 4));
        actors_.emplace_back(enemies_[enemies_.size()-1]);
